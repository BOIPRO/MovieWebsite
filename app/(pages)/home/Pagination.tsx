...
const Pagination = ({ LastPage, onPageChange, currentPage }: pageProps) => {
    ...
    return (
        <div className={`flex justify-center gap-4 py-5 text-sm items-center ${LastPage === 0 ? "hidden": null }`}>
            ...
            <button key={1} className={`w-[40px]  aspect-square cursor-pointer rounded-sm border-white/20 border-[0.2px] ${1 === currentPage ? "bg-amber-600" : "hover:-translate-y-1 hover:shadow-lg active:scale-95 transition-all duration-200"}`} disabled={1 === currentPage}
            ...
            {page.map((e: number) => (
                <button key = {e}  className={`w-[40px] aspect-square cursor-pointer rounded-sm border-white/20 border-[0.2px] ${e === currentPage ? "bg-amber-600" : "hover:-translate-y-1 hover:shadow-lg active:scale-95 transition-all duration-200"}`} disabled={e === currentPage}
