...
export default async function Page({
  ...
  const s = params.s;
   const limit = 30;
  const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/movies/search?s=${s}&page=1&limit=${limit}`,{
    cache : "no-store"
  });
  ...
  return (
    <section className={`max-w-[1200px] mx-auto `}>
        ...
        {totalPages > 0 ?  <ListAnime key={s} listMedia={listMedia} totalPages={totalPages} typeURL={typeUrl} limit={limit}  /> : <div className="w-full text-center text-white text-lg">Page Not Foud</div> }
    </section>
  );
}